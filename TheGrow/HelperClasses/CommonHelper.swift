...
class CommonHelper
{
    ...
    func changefullscreenVC(identifier:String,vc:UIViewController) {
    ...
    func changeScreenVC(identifier:String,vc:UIViewController) {
        let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let vcC: UIViewController = storyboard.instantiateViewController(withIdentifier: identifier) as UIViewController
        vc.present(vcC, animated: true, completion: nil)        
    }
...
//    }
    ...
    class func saveCachedUserData(_ userData:LoginModel){
        ...
        do {
            try userDefaults.setObject(userData, forKey: Constant.User_cache_Key)
        } catch {
      ...
      class func getCachedUserData() -> LoginModel? {
        let userDefaults = UserDefaults.standard
        do {
            let user = try userDefaults.getObject(forKey: Constant.User_cache_Key, castTo: LoginModel.self)
